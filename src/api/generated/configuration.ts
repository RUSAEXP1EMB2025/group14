...
export type ConfigurationOptions<M = Middleware> = Partial<Configuration<M>> &
...
export function mergeConfiguration(
  ...
  return {
    ...
    middleware: mergeMiddleware(
      conf.middleware,
      options?.middleware,
      options?.middlewareMergeStrategy
    )
  };
}
...
function mergeMiddleware(
  ...
  calltimeMiddleware?: Middleware[],
  strategy: 'append' | 'prepend' | 'replace' = 'replace'
) {
...
export function wrapOptions(
  options?: PromiseConfigurationOptions
): StandardConfigurationOptions | undefined {
  if (options) {
    return {
      baseServer: options.baseServer,
      httpApi: options.httpApi,
      middleware: options.middleware?.map(m => new PromiseMiddlewareWrapper(m)),
