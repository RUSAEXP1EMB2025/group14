...
- Google カレンダーから登録された特定のイベント(例:睡眠)を検出・取
  得できること。
...
- パフォーマンス:
  ...
  - LINE からの手動操作や応答指示に対し、遅延なく家電制御を実行でき
    ること。
- 可用性:
  - システムが継続的に稼働し、ユーザーが必要な時にサービスを利用で
    きること。
- 拡張性:
...
- 連携性:
  - Google カレンダー API, LINE Messaging API, Nature Remo Cloud API
    の仕様に正確に準拠して連携できること。
  - 日の入り時刻を取得するための外部サービス(API 等)と連携できる
    こと。
- 環境:
